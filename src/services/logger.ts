...
import { ConfigError, RegexError, RenameError, SearchError } from './errorHandler';
...
export class Logger implements vscode.Disposable {
  ...
  static logWarning(message: string, context?: string): void {
    try {
      const instance = Logger.getInstance();
      if (!instance) {
        console.warn(context ? `[${context}] 警告: ${message}` : `警告: ${message}`);
        return;
      }
  ...
  static logInfo(message: string, context?: string): void {
    try {
      const instance = Logger.getInstance();
      if (!instance) {
        console.info(context ? `[${context}] 情報: ${message}` : `情報: ${message}`);
        return;
      }
  ...
  static logDebug(message: string, context?: string): void {
    try {
      const instance = Logger.getInstance();
      if (!instance) {
        console.debug(context ? `[${context}] デバッグ: ${message}` : `デバッグ: ${message}`);
        return;
      }
   ...
   * ログメッセージをフォーマット
   */
  private formatLogMessage(level: string, message: string, context?: string): string {
   ...
   * OutputChannelにログを出力
   */
  private logToOutputChannel(level: string, message: string, error?: Error, context?: string): void {
    try {
        ...
        if (error instanceof SearchError || error instanceof ConfigError) {
          if (error.cause) {
            ...
            if (error.cause.stack) {
              this.outputChannel.appendLine(`Cause Stack: ${error.cause.stack}`);
            }
          }
        }
      }
   ...
   * コンソールにログを出力
   */
  private logToConsole(level: string, message: string, error?: Error, context?: string): void {
    try {
      const formattedMessage = this.formatLogMessage(level, message, context);
  ...
  private extractErrorDetails(error: Error): Record<string, unknown> {
    ...
    try {
      ...
      if ((error instanceof SearchError || error instanceof ConfigError) && error.cause instanceof Error) {
        details.cause = {
